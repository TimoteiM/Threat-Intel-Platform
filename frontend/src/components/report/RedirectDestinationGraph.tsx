  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 250px), 1fr))", gap: 10 }}>
        {whois && (
          <IntelCard eyebrow="Identity" title="Registration" status={whois.status} accent="#4f6ef7">
        ...
        {(dns || hosting) && (
          <IntelCard eyebrow="Infrastructure" title="Network context" status={hosting?.status || dns?.status} accent="#4f6ef7">
...
function GraphNode({ step, destinationColor }: { step: RedirectStep; destinationColor: string }) {
  const accent = step.role === "source" ? "#4f6ef7" : step.role === "destination" ? destinationColor : "#4f6ef7";
...
function GraphEdge({ status }: { status?: number | string }) {
  return (
    <div style={{ width: 72, position: "relative", height: 30, flex: "0 0 72px" }}>
      <div style={{ position: "absolute", left: 0, right: 8, top: 15, height: 1, background: "#4b759f", boxShadow: "0 0 6px rgba(79,110,247,.3)" }} />
