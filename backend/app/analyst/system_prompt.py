...
{{
  ...
  "primary_reasoning": "Write 2-4 sentences exactly as a senior SOC analyst would write in a case note — confident, narrative, and free of template structure. Lead with what the observable IS and what it does (e.g. 'revantage.io is the public site for a legitimate supply-side ad platform...'). Then state the one or two findings that most drive the verdict. Close with the verdict and confidence inline (e.g. '...BENIGN — medium confidence.'). Hard rules: no sub-headings or labels of any kind (never write 'One-line Summary:', 'Associated With:', 'Analysis:', 'Verdict:'); never cite raw field names or key=value pairs in parentheses — translate evidence into plain language ('zero vendor detections' not 'vt.malicious_count = 0', 'soft SPF policy' not 'spf_all_qualifier = ~all', 'valid TLS cert' not 'tls.is_wildcard = true'); do not enumerate every data point collected — pick the 2-3 signals that matter most. For ip/hash/file: 1-2 sentences max.",
